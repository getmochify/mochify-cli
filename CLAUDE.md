...
- **`--op create` is chosen by flag or by the prompt, not by extension** — its inputs are images, so there is nothing to route on. All files go up in one multipart request (`images` parts, in order) and come back as one PDF, or a zip of one-page PDFs with `--no-combine`. Its prompt mode is `imgpdf`.
- **A prompt that says "pdf" over image inputs means `create`.** `prompt_asks_for_pdf()` in `main.rs` is the CLI's copy of the web app's `/\bpdfs?\b/i` test, which is what switches an image batch there to the `imgpdf` NLP mode; without it `mochify *.jpg -p "convert to pdf"` resolved as an ordinary image request and came back as JPEGs. Both front ends must route the same sentence the same way, so the two tests have to stay in step. `--op` still wins when given.
- **A PDF-in op over image inputs is an error, not a silent squish.** The image path reads none of `--op`, so `mochify photo.jpg --op split` used to answer with a resized image.
